...
import pickle
import os
from orangeclassnetarch_orientation import *
...
def parseDirData(run_dir, seed, resample, val_perc, num_pts, dt = 1):
  ...
  num_samples = 0
  num_samples_dir = {}
  num_samples_dir_size = {}
  for i, trial_dir in enumerate(trial_list):
    trial_list = os.listdir(run_dir+'/'+trial_dir)
    num_samples_i = len(trial_list) - 2
    num_list.append(num_samples_i)
    num_samples_dir[i] = {}
    num_samples_dir[i]['start'] = num_samples
    num_samples_dir[i]['dir'] = trial_dir
    with open(run_dir+'/'+trial_dir+'/orientation_data.pickle','rb') as f:
      data = pickle.load(f,encoding='latin1')
      traj_data.append(data)
    num_samples += num_samples_i
    num_samples_dir[i]['data'] = data
    num_samples_dir[i]['end'] = num_samples
    num_samples_dir[i]['size'] = num_samples_dir[i]['end'] - num_samples_dir[i]['start']
    num_samples_dir_size[i] = num_samples_dir[i]['size']
    # if num_samples_dir[i]['size'] != num_samples_i:
    #   print("Wottt")
    #   exit(0)
  ...
  #order = [k for k, v in sorted(num_samples_dir_size.items(), key=lambda item: item[1])]
  #print(list(num_samples_dir_size.keys()))
  #exit(0)
  val_order = np.ceil(len(num_samples_dir_size)*val_perc).astype(int)
  val_indices = []
  train_indices = []
  val_data = {}
  #val_data["order"] = np.array(order[:val_order])
  val_data["order"] = np.array(random.choices(list(num_samples_dir_size.keys()), k=val_order))
  # print(len(val_data["order"]), val_order)
  # print(val_data["order"])
  for x in val_data["order"]:
    val_indices.extend(list(range(num_samples_dir[x]['start'], num_samples_dir[x]['end'])))
    val_data[x] = num_samples_dir[x]

  for i, x in enumerate(list(num_samples_dir_size.keys())):
    #if i < val_order:
    #  val_indices.extend(list(range(num_samples_dir[x]['start'], num_samples_dir[x]['end'])))
    #  val_data[x] = num_samples_dir[x]
    if x not in val_data["order"]:
      train_indices.extend(list(range(num_samples_dir[x]['start'], num_samples_dir[x]['end'])))
  ...
  return train_indices, val_indices, num_list, traj_data, val_data
...
def loadData(idx,num_list,run_dir,model,traj_data):
  ...
  for ii in idx:
    ...
    waypoints_r.append(waypoint[:,3,:])
    waypoints_p.append(waypoint[:,4,:])
    waypoints_yaw.append(waypoint[:,5,:])
  ...
  return np.array(images), np.array(waypoints_x).reshape(-1,model.num_points,model.bins), np.array(waypoints_y).reshape(-1,model.num_points,model.bins), np.array(waypoints_z).reshape(-1,model.num_points,model.bins), np.array(waypoints_r).reshape(-1,model.num_points,model.bins), np.array(waypoints_p).reshape(-1,model.num_points,model.bins), np.array(waypoints_yaw).reshape(-1,model.num_points,model.bins)
...
def main():
  ...
  # Load in Data
  train_indices, val_indices, num_list, traj_data, val_data = parseDirData(args.data, args.seed, args.resample, val_perc, args.num_pts)
  num_train_samples = train_indices.shape[0]
  num_val_samples = val_indices.shape[0]

  fopen = open('val_data_orientation.pickle', 'wb')
  #print(val_data)
  ...
  val_inputs, val_outputs_x, val_outputs_y, val_outputs_z, val_outputs_r, val_outputs_p, val_outputs_yaw = loadData(val_indices,num_list,data_loc,model,traj_data)
  ...
  with tf.Session() as sess:# Load model if specified 
    ...
    for epoch in range(num_epochs):
      ...
      while batch_idx < num_train_samples:
        ...
        train_inputs, train_outputs_x, train_outputs_y, train_outputs_z, train_outputs_r, train_outputs_p, train_outputs_yaw = loadData(train_indices[batch_idx:end_idx],num_list,data_loc, model,traj_data)
        ...
        feed_dict[model.waypoint_output_r] = train_outputs_r
        feed_dict[model.waypoint_output_p] = train_outputs_p
        feed_dict[model.waypoint_output_yaw] = train_outputs_yaw
      ...
      #val_summary = 0
      val_cost = np.zeros((1,))
      resnet_output = np.zeros((args.num_pts, 6, 0, model.bins)) # 2nd arg for num_waypoints
      ...
      while val_batch_idx < num_validation:
        ...
        val_dict = {model.image_input: val_inputs[val_batch_idx:val_batch_endx], 
            ...
            model.waypoint_output[3]: val_outputs_r[val_batch_idx:val_batch_endx],
            model.waypoint_output[4]: val_outputs_p[val_batch_idx:val_batch_endx],
            model.waypoint_output[5]: val_outputs_yaw[val_batch_idx:val_batch_endx],}
