...
for d in dirs:#files:
    dir_path2 = data_loc + "/" + d
    if os.path.isdir(dir_path2):
        npdir_path2 = np_data_loc + "/" + d
        os.mkdir(npdir_path2)
        print(dir_path2)
        for trial in os.listdir(dir_path2):
            dir_path3 = data_loc + "/" + d + "/" + trial
            if os.path.isdir(dir_path3):
                for status in os.listdir(dir_path3):
                    dir_path = data_loc + "/" + d + "/" + trial + "/" + status
                    if os.path.isdir(dir_path):
                        npdir_path = np_data_loc + "/" + d + "/" + trial + "/" + status
                        os.makedirs(npdir_path)
                        print(dir_path)
                        tmid = time.time()
