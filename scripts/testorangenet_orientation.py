...
def main():
    ...
    parser.add_argument('--val', type=float, default=0.0, help='validation percentage')
    ...
    test_data = SubSet(dataclass,train_idx)
    val_data = SubSet(dataclass,val_idx)

    #Create DataLoaders
    test_loader = DataLoader(test_data,batch_size=args.batch_size,shuffle=False,num_workers=args.j)
    val_loader = DataLoader(val_data,batch_size=args.batch_size,shuffle=False,num_workers=args.j)
    print ('Training Samples: ' + str(len(test_data)))
    ...
    for epoch in range(1):
        ...
        elements = 0.
        for ctr, batch in enumerate(test_loader):
            #print('Batch: ',ctr)
        ...
        #Validation
        #print("Reach here")
        print('Testing Accuracy: ',acc_total)
        #exit()
