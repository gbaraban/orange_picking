...
class Block8(torch.nn.Module):
    def __init__(self, in_f, out_f, stride = 1, downsample=None):
        ...
        self.conv1 = nn.Conv2d(in_channels=in_f,out_channels=out_f,kernel_size=9,stride=1,padding=5)
        self.maxpool1 = nn.MaxPool2d(kernel_size = 9, stride = stride)
        self.conv2 = nn.Conv2d(in_channels=out_f,out_channels=out_f,kernel_size=9,stride=1,padding=4)
...
def make_layer8(in_size,out_size,stride_length=1):
    ...
    downsample = None
    if in_size != out_size or stride_length != 1:
        downsample = nn.Conv2d(in_size,out_size,kernel_size=7,stride=stride_length)
    return nn.Sequential(
...
class OrangeNet8(torch.nn.Module):
    def __init__(self, capacity = 1, num_img = 1, num_pts = 3, bins = 30, mins = None, maxs = None, n_outputs = 3, real_test = False, retrain_off = "", input = 1.0, num_channels = 3):
        ...
        self.conv1 = nn.Conv2d(in_channels=self.num_images*num_channels,out_channels=int(32*self.f),kernel_size=21,stride=4,padding=3)
        self.maxpool = nn.MaxPool2d(kernel_size = 17, stride = 2)
        self.block1 = make_layer8(int(32*self.f),int(48*self.f), stride_length=2)
        ...
        if real_test:
            ...
            linear_layer_size =  63000 #38880 #69120 #55296 #61440 #30720 #34944
