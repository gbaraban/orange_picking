...
class OrangeNet8(torch.nn.Module):
    def __init__(self, capacity = 1, num_img = 1, num_pts = 3, bins = 30, mins = None, maxs = None, n_outputs = 3, real_test = False, retrain_off = "", input = 1.0, num_channels = 3):
        ...
        self.block2 = make_layer8(int(48*self.f),int(96*self.f), stride_length=2)
        self.block3 = make_layer8(int(96*self.f),int(96*self.f), stride_length=1)
        #self.block4 = make_layer8(int(128*self.f),int(128*self.f), stride_length=1)
        #in_size = 768*self.f#122880#temp
        if real_test:
            linear_layer_size = 43008
        else:
            linear_layer_size = 51840 #38880 #69120 #55296 #61440 #30720 #34944
        ...
        self.fc2 = nn.Linear(int(8192*self.f),int(4096*self.f))
        self.dropout2 = nn.Dropout(0.5) #, inplace=True)
        self.fc3 = nn.Linear(int(4096*self.f),int(2048*self.f))
