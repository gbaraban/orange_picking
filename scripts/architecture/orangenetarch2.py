...
class Block8(torch.nn.Module):
    def __init__(self, in_f, out_f, stride = 1, kernel_size=3, downsample=None):
        super(Block8,self).__init__()
        padding = int(kernel_size/2)
        self.conv1 = nn.Conv2d(in_channels=in_f,out_channels=out_f,kernel_size=kernel_size,stride=stride,padding=padding)
        #self.maxpool1 = nn.MaxPool2d(kernel_size = 3, stride = stride)
        self.conv2 = nn.Conv2d(in_channels=out_f,out_channels=out_f,kernel_size=1,stride=1)
...
def make_layer18(in_size,out_size,stride_length=1,kernel_size=3):
    blocks = []
    downsample = None
    ...
    return nn.Sequential(
            Block18(in_size,out_size,stride_length,kernel_size=kernel_size, downsample=downsample),
            Block18(out_size,out_size))

def make_layer8(in_size,out_size,stride_length=1,kernel_size=3):
    blocks = []
    downsample = None
    if in_size != out_size or stride_length != 1:
        downsample = nn.Conv2d(in_size,out_size,kernel_size=1,stride=stride_length)
    return nn.Sequential(
            Block8(in_size,out_size,stride_length,kernel_size=kernel_size,downsample=downsample))#,
...
class OrangeNet8(torch.nn.Module):
    def __init__(self, capacity = 1, num_img = 1, num_pts = 3, bins = 30, mins = None, maxs = None, n_outputs = 3):
        ...
        self.block1 = make_layer8(int(32*self.f),int(64*self.f), stride_length=2, kernel_size=7)
        self.block2 = make_layer8(int(64*self.f),int(128*self.f), stride_length=2)
        self.block3 = make_layer8(int(128*self.f),int(256*self.f), stride_length=2)
        #in_size = 768*self.f#122880#temp
        in_size = 69888*self.f #34944#122880#temp
...
class OrangeNet18(torch.nn.Module):
    def __init__(self, capacity = 1, num_img = 1, num_pts = 3, bins = 30, mins = None, maxs = None, n_outputs = 3):
        ...
        self.layer1 = make_layer18(int(64*self.f),int(64*self.f),kernel_size=5)
