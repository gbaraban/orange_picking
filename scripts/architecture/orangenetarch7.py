...
class Block8(torch.nn.Module):
    def __init__(self, in_f, out_f, stride = 1, downsample=None, l1_kernel=3, l2_kernel=3, l1_padding=2, l2_padding=1, debug=False):
        super(Block8,self).__init__()
        self.debug = debug
        self.conv1 = nn.Conv2d(in_channels=in_f,out_channels=out_f,kernel_size=l1_kernel,stride=stride,padding=l1_padding)
        self.maxpool1 = nn.MaxPool2d(kernel_size = 3, stride = stride)
        self.conv2 = nn.Conv2d(in_channels=out_f,out_channels=out_f,kernel_size=l2_kernel,stride=1,padding=l2_padding)
...
def make_layer8(in_size,out_size,stride_length=1,downsample_kernel=1,downsample_stride=1,l1_kernel=3,l2_kernel=3,l1_padding=2,l2_padding=1):
    blocks = []
    downsample = None
    if in_size != out_size or downsample_stride != 1:
        downsample = nn.Conv2d(in_size,out_size,kernel_size=downsample_kernel,stride=downsample_stride)
    return nn.Sequential(
            Block8(in_size,out_size,stride_length,downsample,l1_kernel, l2_kernel, l1_padding, l2_padding))#,
...
class OrangeNet8(torch.nn.Module):
    def __init__(self, capacity = 1, num_img = 1, num_pts = 3, bins = 30, mins = None, maxs = None, n_outputs = 3, real_test = False, retrain_off = "", input = 1.0, num_channels = 3):
        ...
        self.conv1 = nn.Conv2d(in_channels=self.num_images*num_channels,out_channels=int(128*self.f),kernel_size=15,stride=5,padding=7)
        #self.maxpool = nn.MaxPool2d(kernel_size = 5, stride = 2)
        self.block1 = make_layer8(int(128*self.f),int(256*self.f), stride_length=2, downsample_kernel=1, downsample_stride=2, l1_kernel=9, l2_kernel=9, l1_padding=4, l2_padding=4)
        self.block2 = make_layer8(int(256*self.f),int(512*self.f), stride_length=2, downsample_kernel=1, downsample_stride=2, l1_kernel=7, l2_kernel=7, l1_padding=3, l2_padding=3)
        self.block3 = make_layer8(int(512*self.f),int(1024*self.f), stride_length=2, downsample_kernel=1, downsample_stride=2, l1_kernel=5, l2_kernel=5, l1_padding=2, l2_padding=2)
        self.block4 = make_layer8(int(1024*self.f),int(1024*self.f), stride_length=2, downsample_kernel=1, downsample_stride=2, l1_kernel=3, l2_kernel=3, l1_padding=1, l2_padding=1)
        #in_size = 768*self.f#122880#temp
        if real_test:
            linear_layer_size = 43008
        else:
            linear_layer_size =  49152 #38880 #69120 #55296 #61440 #30720 #34944
        ...
        #self.output = nn.Linear(int(2048*self.f),n_outputs*self.num_points*self.bins)
        self.output = nn.Linear(int(2048*self.f),n_outputs*self.num_points)
