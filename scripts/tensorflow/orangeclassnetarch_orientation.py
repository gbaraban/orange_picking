...
class OrangeClassNet:
  ...
  def __init__(self, capacity = 1, num_img = 2, num_pts = 1, focal_l = -1, min_xyz = (0,-0.5,-0.5), max_xyz = (1,0.5,0.5), bins = 100, dense = 1):
    ...
    self.waypoint_output_r = tf.placeholder(tf.float32,shape=[None,self.num_points,self.bins],name="waypoints_r")
    self.waypoint_output_p = tf.placeholder(tf.float32,shape=[None,self.num_points,self.bins],name="waypoints_p")
    self.waypoint_output_yaw = tf.placeholder(tf.float32,shape=[None,self.num_points,self.bins],name="waypoints_yaw")

    self.waypoint_output = [self.waypoint_output_x, self.waypoint_output_y, self.waypoint_output_z, self.waypoint_output_r, self.waypoint_output_p, self.waypoint_output_yaw]
    #Network Architecture
    self.logits = resnet8(self.image_input,self.num_points,self.bins, f=self.f, reg=self.reg, dense=dense)
    #Training
    self.losses = []#[tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(labels=self.waypoint_output[ii], logits=self.logits[ii])) for ii in range(6)]
