...
def run_DAgger(args,sys_f,env_name,model,data_list=[],j=4,device=None,
        plot_step_flag=False,max_steps=100,dt=0.1,save_path=None,mean_image=None,eps=0.5,physics_hz=50):
    ctr = 0
    ...
    while eps > 5e-3:
        ...
        for step in range(max_steps+1):
            ...
            for ii in range(model.num_images):
                temp_idx = max(0,int(step_time*physics_hz - ii*image_spacing))
